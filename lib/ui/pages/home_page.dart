...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _openCreateGuestPassDialog(DoorRecord? door) async {
    if (door == null) {
      return;
    }
    ...
    final pass = await showDialog<GuestPassRecord>(
      context: context,
      builder: (_) => _CreateGuestPassDialog(
        door: door,
        authService: widget.authService,
      ),
    );
        ...
        actions: [
          TextButton.icon(
            onPressed: () {
              ...
              Navigator.pop(ctx);
              _showMessage('Gecis linki panoya kopyalandi!');
            },
        ...
        children: [
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              onPressed: () => _openCreateGuestPassDialog(selectedDoor),
              icon: const Icon(Icons.share_outlined, size: 18),
              label: const Text('Kurye / Misafir Gecisi Olustur'),
            ),
          ),
        ],
      );
...
class _CreateGuestPassDialog extends StatefulWidget {
  const _CreateGuestPassDialog({
    required this.door,
    required this.authService,
  });
  ...
  @override
  State<_CreateGuestPassDialog> createState() => _CreateGuestPassDialogState();
}

class _CreateGuestPassDialogState extends State<_CreateGuestPassDialog> {
  final _formKey = GlobalKey<FormState>();
  final _titleController = TextEditingController(text: 'Kurye / Misafir');
  String _selectedPreset = 'single_30'; // 'single_30', 'timed_120', 'timed_720'
